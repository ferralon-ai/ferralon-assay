...
func (g *GitCheckout) Fetch(ctx context.Context, repo, revision string) (WorkspacePlan, error) {
	...
	if lang == LangUnknown {
		return cleanup(fmt.Errorf("checkout: cloned repo %q@%q is not a recognized source tree (no go.mod, no .java, no .kt/.kts, no .js/.ts, no .py, and no .cs/.csproj sources)", repo, revision))
	}
