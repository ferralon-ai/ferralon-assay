...
// .js/.ts files) — it returns a single-project WorkspacePlan (the absolute dir + detected
...
func ResolveVendored(relPath string) (WorkspacePlan, error) {
	...
	if lang == LangUnknown {
		return WorkspacePlan{}, fmt.Errorf("checkout: vendored repro %q is not a recognized source tree (no go.mod, no .java, and no .js/.ts sources)", abs)
	}
