// Command tegron-plugin-js is the out-of-process JavaScript/TypeScript language
// analysis subprocess (inv.8). It reads exactly one newline-delimited JSON
// plugin.Request from stdin, dispatches on Op to the in-process jsanalysis functions,
...
// source-level analysis. resolve_versions reads installed versions from npm/yarn/pnpm
// lockfiles; reachability and compute_taint derive source→sink paths over the lexical
...
// run reads one Request, dispatches, and writes one Response. It returns a non-nil
// error only for a hard failure (the caller exits non-zero); in that case it still
// writes a Response with Error set so the client can decode a structured failure.
...
func dispatch(ctx context.Context, req plugin.Request) (plugin.Response, error) {
	...
	case plugin.OpIndexSymbols:
		...
		res, err := jsanalysis.IndexSymbols(ctx, *req.IndexSymbols)
		if err != nil {
	...
	case plugin.OpResolveSymbols:
		...
		res, err := jsanalysis.ResolveDependencySymbols(ctx, *req.ResolveSymbols)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpResolveVersions:
		...
		res, err := jsanalysis.ResolveDependencyVersions(ctx, *req.ResolveVersions)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpCallGraph:
		...
		res, err := jsanalysis.CallGraph(ctx, *req.CallGraph)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpFindIngresses:
		...
		res, err := jsanalysis.FindIngresses(ctx, *req.FindIngresses)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpReachability:
		...
		res, err := jsanalysis.Reachability(ctx, *req.Reachability)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpComputeTaint:
		...
		res, err := jsanalysis.ComputeTaint(ctx, *req.ComputeTaint)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpGenerateHarness:
		...
		res, err := jsanalysis.GenerateHarness(ctx, *req.GenerateHarness)
		if err != nil {
			return plugin.Response{}, err
		}
	...
	case plugin.OpBuildManifest:
		...
		res, err := jsanalysis.BuildManifest(ctx, *req.BuildManifest)
		if err != nil {
			return plugin.Response{}, err
		}
...
// writeError writes a structured error Response (so the client decodes a clean
// failure) and returns a non-nil error so main exits non-zero (inv.4).
