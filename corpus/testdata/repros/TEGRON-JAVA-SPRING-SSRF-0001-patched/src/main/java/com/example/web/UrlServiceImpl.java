...
/**
 * UrlServiceImpl is the PATCHED variant of the concrete SSRF sink reached through
 * the @Autowired UrlService interface field on FetchController. It is byte-for-byte
 * the same sink shape as the vulnerable variant EXCEPT resolveTarget enforces an
 * allowlist: the "internal"/metadata pseudo-host is rejected BEFORE any seed is
 * read and before any outbound request is made.
 *
 ...
 * fired beacon a sound proof rather than a generic exfil any build would emit.
 *
 * The call graph still connects @RestController → interface → impl (so a candidate
 * pair still forms); the patched build is dark only because the sink's runtime
 * allowlist forecloses the beacon — exactly the live gate's negative control.
 */
@Service
