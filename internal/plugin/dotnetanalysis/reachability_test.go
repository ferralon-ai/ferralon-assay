...
// real lexical reachability connects the [HttpGet] controller ingress (Handle) through the
// utility hop (Fetch) to the sink (OpenConn), emitting the ingress→sink trace — AND declares
// Partial(dynamic_dispatch) even though a path WAS found. C# static reachability is a
// candidate narrower, never an adjudicator, so it must NEVER return Complete.
func TestReachability_IngressToSinkPathIsAlwaysPartial(t *testing.T) {
	...
	if p.Ingress == "" {
		t.Fatal("path must carry a non-empty ingress (the controller action entry)")
	}
	...
	if len(p.Trace) < 3 {
		t.Fatalf("expected the multi-hop trace Handle->Fetch->OpenConn; got %v", p.Trace)
	}
	// The critical C# invariant: a reachable path is STILL Partial (never Complete).
	if res.Partiality.Complete {
		t.Fatal("C# reachability must NEVER return Complete, even when a path is found (structurally weak)")
	}
	if !hasReason(res.Partiality, plugin.PartialReasonDynamicDispatch) {
		t.Fatalf("C# reachability must always carry dynamic_dispatch; got %v", res.Partiality.Reasons)
	}
}
